class ApplicationsController < ApplicationController
  ...
  def create
    ...
    if params[:todo] == 'apply' 
      ...
      success_message = 'Project successfully saved. You can find your saved project under "Saved projects" tab in your dashboard.'
      fail_message = 'Project could not be saved.'
      status = params[:todo]
    elsif params[:todo] == 'save'
      success_message = 'Application successfully saved. You can find your work-in-progress application under "My applications" tab in your dashboard.'
      fail_message = 'Application could not be saved.'
      status = params[:todo]      
    else
    ...
    respond_to do |format|
      ...
      if status == 'shortlist' || status == 'apply'
        if status == 'shortlist'
  ...
  def applicant_update
    ...
    if params[:todo] == 'engage'
        ...
        @application.assign_attributes(application_update_params)
        ...
        @application.save(validate:false)
        @application.statuses= params[:todo]
        message = "Application saved. You can find your work-in-progress applications in \"My applications\" tab in your dashboard."
        successFlag = 1
    end 
  ...
  def read
    ...
    @role = current_user.role
    if @role == 'npo'
        @applications = current_user.applications.order('created_at ASC').where("applications.status not in ('shortlist','save')")
  ...
  def application_update_params
    params.require(:application).permit(:message, :est_completion_date, :open_questions, :required_resources, :user_id)
  end
